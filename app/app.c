...
/**************chenshuhua***************************/
//#define USER_SCNRSP_DATA        "\x0c\x08\x46\x46\x46\x46\x46\x46\x46\x46\x46\x46\x47" //BK3411-GATT"//初始名字  前面2个字符不是名字别动
//void ReadBleName(void);
...
void appm_init()
{
    ...
    //SetBleName(USER_SCNRSP_DATA,13);
...
//        ReadBleName();
//        
//        if(( ADVbuffer[0]=='\0')||( ADVbuffer[0]==0xff))
//        {
//        
...
//            
//            ADVNum=0;
//            ADVNum=strlen(ADVbuffer);
//            cmd->info.host.scan_rsp_data_len = 0;
//            memcpy(&cmd->info.host.scan_rsp_data[cmd->info.host.scan_rsp_data_len],
//                   ADVbuffer, ADVNum);
//            cmd->info.host.scan_rsp_data_len += ADVNum;
//        }
...
/**************chenshuhua***************************/
void SetBleName(unsigned char *name,int len){
    uint8_t testadv[31];
    memset(testadv,'\0',31);
    memcpy(testadv,name,len);
    flash_erase(FLASH_SPACE_TYPE_MAIN,0x00027800,31);
    flash_write(FLASH_SPACE_TYPE_MAIN,0x00027800,31,testadv);
    
}
void ReadBleName(void)
{

    //flash_read(FLASH_SPACE_TYPE_MAIN,0x00027800,31,ADVbuffer);
